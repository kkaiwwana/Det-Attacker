...
def depth_to_local_point_cloud(image, color=None, max_depth=0.9, min_depth=0.0, mode='depth'):
    """
    Convert an image containing CARLA encoded depth-map to a 2D array containing
    ...
    if mode == 'depth':
        ...
        normalized_depth = depth_to_logarithmic_grayscale(image)[:, :, 0] / 255.0
    ...
    #     return carla.Sensor.PointCloud(
    #         image.frame_number,
    #         numpy.transpose(p3d),
    #         color_array=color)
    # # [[X1,Y1,Z1],[X2,Y2,Z2], ... [Xn,Yn,Zn]]
    # return carla.Sensor.PointCloud(image.frame_number, numpy.transpose(p3d))
